function [submap_coordinates] = ...
    get_submap_coordinates(pos_env, submap_edge_size, map_params)
% Gets grid coordinates of observed submap of the grid map.

pos_grid = env_to_grid_coordinates(pos_env, map_params);
...
while (xr > map_params.dim_x)
    xr = xr - 1;
end
yu = pos_grid(1) + floor(submap_edge_size.y/2);
while (yu > map_params.dim_y)
    yu = yu - 1;
end
